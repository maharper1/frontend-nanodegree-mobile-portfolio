...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    ...
    //--- switch(size) directly computes the new pizza size.
    ...
    //--- Simplify to directly set new element size.
...
function updatePositions() {
  ...
  //--- Changed to use getElementsByClassName from document.querySelectorAll('.mover');
  var items = document.getElementsByClassName('mover');
  //--- Compute everything possible outside of the for loop.
  var phaseConstant = document.body.scrollTop / 1250;
  var phase0 = 100 * Math.sin(phaseConstant + 0);
  var phase1 = 100 * Math.sin(phaseConstant + 1);
  var phase2 = 100 * Math.sin(phaseConstant + 2);
  var phase3 = 100 * Math.sin(phaseConstant + 3);
  var phase4 = 100 * Math.sin(phaseConstant + 4);

  //--- Stepping by 5, since the moves work in concert for groups of 5.
  for (var i = 0; i < items.length; i=i+5) {
    var change0 = items[i].basicLeft + phase0 - items[i].left;
    var change1 = items[i+1].basicLeft + phase1 - items[i+1].left;
    var change2 = items[i+2].basicLeft + phase2 - items[i+2].left;
    var change3 = items[i+3].basicLeft + phase3 - items[i+3].left;
    var change4 = items[i+4].basicLeft + phase4 - items[i+4].left;
    //--- Used transform/translateX instead of setting left directly.
    items[i].style.transform = "translateX(" + change0 + "px)";
    items[i+1].style.transform = "translateX(" + change1 + "px)";
    items[i+2].style.transform = "translateX(" + change2 + "px)";
    items[i+3].style.transform = "translateX(" + change3 + "px)";
    items[i+4].style.transform = "translateX(" + change4 + "px)";
  }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  //--- Not sure that this matters, but I create the entire movers div, including
  //--- all of the pizzas, before appending the whole thing to the document,
  //--- rather than appending each individula pizza to the document.
  var MoversDiv = document.createElement("div");
  MoversDiv.id = "movingPizzas1";
  //--- The setting was originally "col=md-6", but this results in no pizzas on the
  //--- left hand side for some screen sizes due to column wrapping.
  MoversDiv.classList.add("col-md-12");

  //--- Compute max possible rows, given screen height.
  var rows = Math.ceil(screen.height/s);
  //--- Total pizzas needed computed.
  var PizzasNeeded = cols * rows;
  //--- updatePositions is simpler if the count is divisible by 5.
  //--- Compute the remainder when the number of pizzas is divided by 5.
  var diff = PizzasNeeded % 5;
  //--- Set maxPizzas to the next larger number that is divisible by 5.
  var maxPizzas;
  if (diff != 0){
    maxPizzas = PizzasNeeded + 5 - (diff % 5);
  } else {
    maxPizzas = PizzasNeeded;
  }

  //--- Reduced the total number of pizzas created from the original of 200.
  for (var i = 0; i < maxPizzas; i++) {
    var elem = document.createElement('img');
    elem.className = 'mover';
    //--- scaled the pizza image to the needed size
    elem.src = "images/pizza-100.png";
    elem.style.height = "100px";
    //--- Bothered me that the pizzas weren't round!
    //--- elem.style.width = "73.333px";
    elem.style.width = "100px";
    elem.basicLeft = (i % cols) * s;
    //--- Set starting left value to 0 for use later with the transform function,
    //--- since transform moves the indicated amount rather than setting the position itself.
    elem.left = 0;
    elem.style.top = (Math.floor(i / cols) * s) + 'px';
    //--- Append all of the moving pizzas at once.
    MoversDiv.appendChild(elem);
  }
  document.getElementById("header").appendChild(MoversDiv);
  updatePositions();
});
